...
-- 테이블 스페이스 : 객체와 Log를 저장하는 물리적인 파일
    ...
    -- Log : Transation Log 를 저장
    
    -- DataFile 과 Log 파일은 물리적으로 다른 하드공간에 저장해야 성능을 높일 수 있다.
...
-----------------------------------------------
-- 문제 : usertest02 계정을 생성 후에 users 테이블 스페이스에서 테이블 (tbl2) 생성후 insert.
...
select * from all_tables    -- 테이블의 소유주를 출력해 준다. 계정별로 소유한 테이블을 출력할 수 있다.
